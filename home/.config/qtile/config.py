...
def autostart():
    home = os.path.expanduser('~/.config/qtile/autostart.bash')
...
keys = [
    ...
    Key([], "XF86AudioRaiseVolume", lazy.widget["volume"].increase_vol(), desc="Lower volume"),
...
layouts = [
    layout.Bsp(fair=False, border_focus_stack=["#d75f5f", "#8f3d3d"], border_width=4, margin=2, margin_on_single=0),
    layout.Max(),
    ...
    Screen(
        bottom=bar.Bar(
            [
                ...
                widget.GroupBox(rounded=False, highlight_method='block', hide_unused=True, fontsize=20),
                widget.Prompt(),
                widget.WindowName(),
                widget.Chord(
                    chords_colors={
                ...
                widget.Volume(
                    ...
                    get_volume_command="pactl get-sink-volume @DEFAULT_SINK@ | grep -Po '\\d+(?=%)' | head -n 1 | sed 's/$/%/g'",
